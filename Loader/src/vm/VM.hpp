...
struct RFLAGS {
    union {
        struct {
            std::uint64_t carry : 1;          // CF (Carry Flag)
            std::uint64_t reserved1 : 1;     // Reserved (always 1)
            std::uint64_t parity : 1;        // PF (Parity Flag)
            std::uint64_t reserved2 : 1;     // Reserved (always 0)
            std::uint64_t adjust : 1;        // AF (Adjust Flag)
            std::uint64_t reserved3 : 1;     // Reserved (always 0)
            std::uint64_t zero : 1;          // ZF (Zero Flag)
            std::uint64_t sign : 1;          // SF (Sign Flag)
            std::uint64_t trap : 1;          // TF (Trap Flag)
            std::uint64_t interrupt : 1;     // IF (Interrupt Enable Flag)
            std::uint64_t direction : 1;     // DF (Direction Flag)
            std::uint64_t overflow : 1;      // OF (Overflow Flag)
            std::uint64_t iopl : 2;          // IOPL (I/O Privilege Level)
            std::uint64_t nested : 1;        // NT (Nested Task)
            std::uint64_t reserved4 : 1;     // Reserved (always 0)
            std::uint64_t resume : 1;        // RF (Resume Flag)
            std::uint64_t virtual8086 : 1;   // VM (Virtual 8086 Mode)
            std::uint64_t alignment : 1;     // AC (Alignment Check)
            std::uint64_t virtualint : 1;    // VIF (Virtual Interrupt Flag)
            std::uint64_t virtualintpending : 1; // VIP (Virtual Interrupt Pending)
            std::uint64_t cpuid : 1;         // ID (CPUID available)
            std::uint64_t reserved5 : 40;    // Reserved
        };
        std::uint64_t rflags; // Whole RFLAGS register as a single 64-bit integer
    };
};
